...
int RSQ(int v, int tl, int tr, int l, int r)
{
    ...
    int left_ans = RSQ(2*v, tl, tm, l, r);              // for partial overlap
    int right_ans = RSQ(2*v+1, tm+1, tr, l, r);
...
void update_node(int v, int tl, int tr, int val, int pos)
{
    ...
    tree[v]=tree[2*v]+tree[2*v+1];  // update whole segment tree
    return;
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        while(q--)
        {
            ...
            int sum = RSQ( 1, 0, n-1, l, r);    // Range sum query
            cout << sum << endl;
        }
        int inc=2;
