...
{
    if(x>=0 and x<m and y>=0 and y<n and maze[x][y]==-1) return true;
    return false;
}
bool tour(int a,int b,int mov)
{
    int i,x,y;
    if(mov==m*n) return true;
...
{
    int a,b,i,j,k;
    scin2(m,n); // number of rows and columns  // n<=9 , m<=9
    scin2(a,b); // index of starting position( ZERO based )
    ...
    {
        pf("Knight can't complete a closed tour\n");
    }
