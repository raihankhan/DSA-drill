...
int bfs(int node,int mod)
{
    queue<int>q;
    int par[n+3]={0};
    int level[n+3]={0},max_level=0,new_node=node;
    q.push(node);
    level[node]=0;
    par[node]=-1;
    ...
    while(!q.empty())
    {
        ...
        for(auto v:g[node])
        {
            if(v!=par[node])
            {
                ...
                if(max_level<=level[v]) max_level=level[v],new_node=v;
                q.push(v);
            }
        }
    }
    ...
    if(mod==1)
    {
        ...
        rep(i , 1 , n+1)
        {
            pf("%d\n",max(max_level,max(level[i]+1 , rlevel[i]+1)) );
        }
    }
    else return bfs(new_node,1);
}
