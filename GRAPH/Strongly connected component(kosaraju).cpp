...
using namespace std;
veci edges[p5],redges[p5],components[p5];
bool visited[p5];
stack<int>s;
int m,n,com;
...
void dfs2(int u)
{
    ...
    rep(v , 0 , redges[u].size())
    {
        if(!visited[redges[u][v]])
            dfs2(redges[u][v]);
    }
}
...
int main()
{
   ...
   cout << "Number of components " << com << endl;

   rep(i , 1 , com+1)
   {
       rep(j , 0 , components[i].size())
       {
           cout << components[i][j] << " ";
       }
       pfn;
