...
///...............................Code Starts Here........................................
vector<int>shirt_owner[107];
int allmasked;
lli dp[1050][107];
bool is_set(int mask,int j)
{
    if(mask&(1<<j)) return 1; return 0;
}
lli solve(int mask,int shirt_id)
{
    if(mask==allmasked) return 1;
    if(shirt_id>100) return 0;
    lli &ret = dp[mask][shirt_id];
    if(ret!=-1) return ret;
    lli ways=solve(mask,shirt_id+1);
    int i,p;
    for(i=0;i<shirt_owner[shirt_id].size();i++)
    {
        if(!is_set(mask,shirt_owner[shirt_id][i]))
        {
        ways+=solve(mask|(1 << shirt_owner[shirt_id][i]),shirt_id+1);
        }
        ways%=M;
    }
...
int main()
{
    ...
    while(t--)
    {
        ...
        lli ways = solve(0,1);
        cout << ways << endl;
        for(i=1;i<=100;i++) shirt_owner[i].clear();
