...
void printlps(string s,int start,int k)
{
    int i;
...
int lps(string s)  // dp approach ... complexity O(n^2)
{
    int len=s.length(),k,i,j,p,maxi,start;

    bool memo[len+1][len+1];
    mem(memo,0);
...
int main()
{
   string s;
   cin >> s ;
   cout << "Length of longest palindromic substring is : "  << lps(s) << endl;
